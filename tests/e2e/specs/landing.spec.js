...
describe('Test landing link click', () => {
  ...
  test('Search box behaviour', () => {
    ...
    cy.get('.searchbox-button')
      .click();
    cy.wait(300);
    cy.url()
      .should('include', 'search/miku');
    ...
    cy.get('.search-suggestion-items')
      .should('be.visible');
    cy.get('.search-header-close')
      .click();
    cy.url()
      .should('not.include', 'search/miku');
    cy.wait(300);
    cy.get('.banner')
      .should('be.visible');
  });
  ...
  test('Announcement behaviour', () => {
    cy.intercept('GET', 'pixiviz/announcement.json', { fixture: 'testAnnouncement.json' }).as('testReq');
    cy.clearLocalStorage();
    cy.visit('/');
    cy.wait(['@testReq']);
    cy.wait(300);
